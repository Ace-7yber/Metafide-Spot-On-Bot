...
def predict_spots(current_price: float, up_range: float = 15, down_range: float = 15, gid: str = "0"):
    ...
    if context is not None:
        ...
        if ai_model is not None:
            # We feed the live RSI and MA distance to the Random Forest
            features = pd.DataFrame([[rsi, ma_distance]], columns=['rsi', 'ma25_distance'])
            prediction = ai_model.predict(features)[0]
            
            # The model outputs 1 for Bullish, 0 for Bearish
            if prediction == 1:
...
if __name__ == "__main__":
    uvicorn.run("brain:app", host="0.0.0.0", port=8000, log_level="warning")